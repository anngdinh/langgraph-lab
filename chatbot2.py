...
def log_state(state: AgentState):
    ...
    for message in state["messages"]:
        print(type(message))
        if isinstance(message, ToolMessage) is False:
...
async def main():
    async with AsyncSqliteSaver.from_conn_string(":memory:") as memory:
        abot = Agent(model, [tool], system=prompt, checkpointer=memory)
        messages = HumanMessage(
            # content="who won the last soccer world cup and what is 5 fact about that country?"
            # content= "Who won the last football world cup and what is the current population of that country?"
            # content= "Who won the last football world cup and what is the current population of that country?"
            content= "Who won the last World Cup and tell us about that country's journey to the championship?"
        )
        thread = {"configurable": {"thread_id": "1234"}}
